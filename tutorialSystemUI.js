"use strict";
/* =========================================================================
   Tutorial System UI 레이어 — tutorialSystem.js에서 분리된 안내/완료 팝업
   DOM 렌더링. 튜토리얼 진행 상태 추적(localStorage 플래그, 전투 후킹)은
   tutorialSystem.js에 남아있고 이 파일은 팝업을 그리기만 한다.
   tutorialSystem.js가 로드되자마자(즉시 실행 IIFE) 시작 화면의
   ".start-tutorial-button" 클릭 핸들러를 이 파일의 showTutorialGuidePopup로
   바로 연결하므로, 이 파일은 반드시 tutorialSystem.js보다 먼저 로드되어야
   한다(index.html 스크립트 순서 참고).
